...
import { CheckCircle2, Link, Lock, Mail, User, UserPlus } from 'lucide-react';
    ...
    <div className="grid grid-cols-2 gap-4">
      <button
        ...
        className={`flex flex-col items-center justify-center gap-2 p-4 sm:p-6 rounded-2xl border-2 transition-all duration-200
          ${activeTab === "signin"
            ? "border-[#a3df44] bg-[#22252a]"
            : "border-transparent bg-[#2c2e33] hover:bg-[#32353b]"
          }`}
      >
        <User className={`w-8 h-8 sm:w-10 sm:h-10 ${activeTab === "signin" ? "text-[#a3df44]" : "text-white"}`} />
      ...
      >
        <UserPlus className={`w-8 h-8 sm:w-10 sm:h-10 ${activeTab === "signup" ? "text-[#a3df44]" : "text-white"}`} />
